...
(function () {
  ...
  function configured(panel) { return !!(PANELS[panel] && parse(process.env[PANELS[panel]])); }
  ...
  async function unlock(panel, user, code, ip) {
    ...
    const spec = parse(process.env[PANELS[panel]]);
    if (!spec) return { ok: false, status: 503, error: 'This panel is locked: ' + PANELS[panel] + ' is not set on the server.' };
    code = String(code || '');
    ...
    const good = k.length === spec.hash.length && crypto.timingSafeEqual(k, spec.hash);
  ...
  function lockedMessage(panel) {
    return configured(panel) ? ('The ' + panel + ' panel is locked. Enter its code.') : ('The ' + panel + ' panel is locked: ' + PANELS[panel] + ' is not set on the server.');
  }
