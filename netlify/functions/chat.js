...
const KARAM_BASE = [
  ...
  'BIG FILES AND FOLDERS (deliver fast): for a large file or folder, do NOT rewrite the whole thing. Return only what changed as labeled patches - for each: the filename, the exact existing snippet to find, and the exact replacement. Keep output small and focused on the changes only. When a full file is genuinely needed, deliver it as one labeled code block with its filename on the opening line. For many files, keep them together and return them as a single archive. Generate the least text that fully does the job - that is what makes big work come back fast.', 
